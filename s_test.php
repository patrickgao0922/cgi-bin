 ...
 */
    ...
    {
        if (!file_exists('/home4/igloosof/public_html/iGloo/database/'.$directory)) 
        {
            mkdir ('/home4/igloosof/public_html/iGloo/database/'.$directory, 0777, true);
        }
 	...
 	{
            $command = $command."\t".$deviceToken;
        }

        file_put_contents ('/home4/igloosof/public_html/iGloo/database/'.$directory."/".$name, $commandPrefix.$command);
	...
	$directoryPath = '/home4/igloosof/public_html/iGloo/database/'.$directory.'/';
	...
	$GET_String = '?entry='.$mapEntry.'&serial='.$mapSerial.'&suburb='.$mapSuburb.'&lat='.$mapLat.'&lon='.$mapLon.'&installed_date='.$mapInstalled_date;
	
	$addEntryURL = 'http://igloohomecontrol.com/iGloo/cgi-bin/addentry.php';
	
	$addEntryURLWith_GET = $addEntryURL.$GET_String;
 ...
 */
    ...
    function processAlerts($pDebug, $pCodeID, $originalDeviceToken)
    {
        ...
        $deviceURL = '/home4/igloosof/public_html/iGloo/database/'.$pCodeID.'/dd.xml';
        ...
        if (file_exists($deviceURL))
        {
            ...
            if ($deviceData)
            {
                ...
                if (file_exists('/home4/igloosof/public_html/iGloo/database/'.$pCodeID.'/DState.ds'))
                {
                    $currentState = file_get_contents('http://igloohomecontrol.com/iGloo/database/'.$pCodeID.'/DState.ds');
...
//
	            ...
	            {
	            	//echo '['.$originalDeviceToken.', '.$currentDeviceToken.'],   ';
	                alertClient($pDebug, $currentDeviceToken, $alertMessage, $soundFile, $zoneID, $deviceID, $pCodeID, 'ck.pem');
	                //alertClient($pDebug, $currentDeviceToken, $alertMessage, $soundFile, $zoneID, $deviceID, $pCodeID, 'iGlooPush.pem');
	            }
	        }
	    }
	}
